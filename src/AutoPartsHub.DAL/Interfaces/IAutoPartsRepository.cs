...
/// <summary>
/// Описывает параметры запроса, который DAL преобразует в LINQ к базе данных.
/// </summary>
/// <remarks>
/// Это не пользовательский DTO: тип содержит уже проверенные BLL параметры и
/// остаётся рядом с репозиторием, чтобы Presentation Layer не зависел от деталей
/// построения запроса к хранилищу.
/// </remarks>
...
/// </summary>
/// <remarks>
/// Для учебного MVP один репозиторий одновременно играет роль фасада над
/// хранилищем и Unit of Work. Это позволяет сценарию оформления заказа одной
/// транзакцией изменить корзину, остатки и заказ. При росте независимых модулей
/// интерфейс можно разделить на специализированные репозитории без изменения
/// Telegram-команд.
/// </remarks>
