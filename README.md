...
flowchart TD
    ...
    BLL --> DAL
    BLL --> DTOs
    BLL --> Models["AutoPartsHub.Models<br/>POCO-сущности и перечисления"]
    DAL --> Models
    DTOs --> Models
    DAL --> DB[(PostgreSQL)]
~~~

- **Models** — POCO-сущности с открытыми свойствами, связями и перечислениями,
  но без бизнес-логики;
...
- **DAL** (Data Access Layer) — небольшие интерфейсы и реализации репозиториев,
  `UnitOfWork`, `DbContext` и миграции Code First;
- **TelegramBot** (Presentation Layer) — команды Telegram, консольный режим и
  реализация канала уведомлений;
- **Tests** — модульные тесты правил BLL и ограничений архитектуры.

Структура повторяет учебный подход из проекта `DeliveryApp`: модели являются
простыми объектами данных с открытыми `get/set`, а их создание и изменение
выполняет BLL. У BLL и DAL есть каталоги `Interfaces`, `Services`/`Repositories`
и собственные классы-регистраторы.
...
При этом классы BLL не используют `DbContext`, Npgsql и реализации репозиториев —
...
|---|---|---|
| `Models` | POCO-сущности, навигационные свойства и enum | EF Core хранит данные, а модели не смешиваются с бизнес-сценариями |
...
| `BLL/Services` | бизнес-сценарии | здесь координируются модели и репозитории |
| `BLL/Rules` | проверка и изменение моделей | модели остаются только данными, а правила можно тестировать без БД |
| `BLL/RegistratorServices` | DI-регистрация BLL | `Program` не знает конкретные реализации |
| `DAL/Interfaces` | специализированные контракты репозиториев, `IUnitOfWork` и инфраструктуры | BLL получает только необходимые операции и соблюдает ISP |
| `DAL/Context` | `DbContext` и миграции | все детали EF Core собраны вместе |
| `DAL/Repositories` | отдельные EF-репозитории и `UnitOfWork` | BLL не видит реализацию хранения, а общий `DbContext` сохраняет согласованные изменения |
...
### Как соблюдается SOLID

- **SRP**: модели хранят данные, BLL Rules проверяют и изменяют их, сервисы
  координируют сценарии, каждый DAL-репозиторий отвечает за свою область;
- **OCP**: другой канал представления или реализацию хранилища можно подключить
  через интерфейсы и DI без изменения Telegram-команд;
- **LSP**: BLL использует только контракты, поэтому тестовая или альтернативная
  реализация может заменить EF-репозиторий без изменения сценария;
- **ISP**: вместо одного большого репозитория используются `IUserRepository`,
  `ICatalogRepository`, `ICartRepository`, `IOrderRepository` и другие небольшие
  интерфейсы; сервис не получает методы чужой области;
- **DIP**: Telegram зависит от интерфейсов BLL, а BLL — от интерфейсов DAL,
  но не от `DbContext`, Npgsql и конкретных репозиториев.

## Документация исходного кода

Созданные вручную классы, интерфейсы, перечисления, записи, свойства и методы
снабжены XML-комментариями `summary`. Комментарии у моделей объясняют свойства
и связи, а комментарии BLL дополнительно обосновывают размещение правил вне
POCO-сущностей. Для позиционных `record` назначение
...
XML-документации будет обнаружена при обычной сборке. Схема в автоматически
созданных EF Core файлах `Context/Migrations` вручную не изменяется; добавленные
XML-комментарии только поясняют назначение сгенерированных методов.
...
В Telegram дополнительно отображается постоянная reply-клавиатура с кнопками
`Каталог`, `Категории`, `Корзина`, `Мои заказы`, `Уведомления` и `Помощь`.
Presentation-слой преобразует нажатие кнопки в уже существующую slash-команду,
поэтому UI не дублирует бизнес-сценарии и не влияет на консольный режим. Команды,
которым нужны VIN, артикул, количество или данные доставки, остаются текстовыми:
их точный формат всегда доступен через кнопку `Помощь` и системное меню `/`.
...
| `/addcompatibility АРТИКУЛ\|Марка\|Модель\|ГОД_ОТ\|ГОД_ДО\|ДВИГАТЕЛЬ` | назначить товару совместимость с автомобилем | подтверждение артикула, автомобиля и диапазона годов |
| `/updateproduct АРТИКУЛ\|ЦЕНА\|ОСТАТОК` | изменить цену и складской остаток | актуальные цена и остаток товара |
| `/deactivateproduct АРТИКУЛ` | выполнить мягкое удаление товара | подтверждение, что товар скрыт из каталога |
| `/activateproduct АРТИКУЛ` | восстановить мягко удалённый товар | подтверждение возврата товара в каталог |
...
Интерфейсы `IUserRepository`, `ICatalogRepository`, `ICartRepository`,
`IOrderRepository`, `IVehicleRepository`, `ISubscriptionRepository` и
`INotificationRepository` находятся в `DAL/Interfaces`, а реализации — в
`DAL/Repositories`. `IUnitOfWork` задаёт одну границу сохранения и транзакции для
общего scoped `DbContext`. BLL не обращается к `DbContext` напрямую. Модели не
содержат методов валидации или изменения состояния: эти
операции выполняют классы `BLL/Rules`, которые вызываются сервисами. Цена и
название копируются в `OrderItem` при оформлении: старый заказ
...
Администратор сначала создаёт карточку товара командой `/addproduct`, а затем
может добавить одно или несколько правил применимости через
`/addcompatibility`. Цену и остаток можно изменить командой
`/updateproduct`; `/deactivateproduct` выполняет мягкое удаление, а
`/activateproduct` возвращает товар в каталог. Команда сохраняет отдельную строку в
`ProductCompatibilities`; VIN-поиск сопоставляет марку, модель, год и двигатель
сохранённого автомобиля с этими правилами.
...
6. Коллекции заменены интерфейсами репозиториев и PostgreSQL, консоль — Telegram API.
